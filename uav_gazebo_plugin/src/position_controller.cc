...
class position_controller : public ModelPlugin {
    /// \brief Constructor
public:
    position_controller() { }
...
public:
    virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
    {
        ...
        this->model->GetJointController()->SetPositionPID(joint_name, this->pid);

        // Set the joint's target position.
        this->model->GetJointController()->SetPositionTarget(joint_name, position);

        // Initialize ros, if it has not already bee initialized.
        if (!ros::isInitialized()) {
            int argc = 0;
            char** argv = NULL;
            ros::init(argc, argv, joint_name_ori + "_" + "node", ros::init_options::NoSigintHandler);
        }
        ...
        ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Float32>(
            "/" + this->model->GetName() + "/" + joint_name_ori + "/pos_cmd", 1,
            boost::bind(&position_controller::OnRosMsg, this, _1), ros::VoidPtr(), &this->rosQueue);
        this->rosSub = this->rosNode->subscribe(so);

        // Spin up the queue helper thread.
        this->rosQueueThread = std::thread(std::bind(&position_controller::QueueThread, this));
    }
...
public:
    void SetPosition(const double& _pos)
    {
        ...
        this->model->GetJointController()->SetPositionTarget(joint_name, _pos);
    }
...
public:
    std::string joint_name, joint_name_ori;
};

// Tell Gazebo about this plugin, so that Gazebo can call Load on this plugin.
GZ_REGISTER_MODEL_PLUGIN(position_controller)
}
#endif